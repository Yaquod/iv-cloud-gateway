...
// cppcheck-suppress unusedFunction
void cloud_gateway::MqttClient::mqtt_publish(
...
void cloud_gateway::MqttClient::mqtt_subscribe(const std::string& topic) {
  ...
  client_.async_subscribe(
      ...
      [this, topic](boost::mqtt5::error_code ec,
        ...
        if (ec)
          spdlog::error("failed to subscribe to topic {} :{}", topic,
                        ec.message());
        else
...
void cloud_gateway::MqttClient::set_message_arrived_handler(
    MessageArrivalCallback callback) {
  cb = std::move(callback);
}

void cloud_gateway::MqttClient::start_receive_loop() {
  client_.async_receive(
      [this](boost::system::error_code ec, std::string topic,
             std::string payload, boost::mqtt5::publish_props) {
        if (ec) {
