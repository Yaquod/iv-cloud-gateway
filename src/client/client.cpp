...
//   auto channel =
//       grpc::CreateChannel(server_add, grpc::InsecureChannelCredentials());
...
//
//   cq_thread = std::thread(&VechileGatewayClient::ProcessCompletionQueue,
//   this);
// }
//
...
//       } else {
//         spdlog::error("[CLIENT]Async call failed - Status: {}",
...
// }
//
// bool VechileGatewayClient::UpdateLocation(const std::string &vin_number_val,
//                                           double long_val, double lat_val) {
//   vehicle_gateway::UpdateVehicleLocationRequest req;
//   req.set_vinnumber(vin_number_val);
//   req.set_longitude(long_val);
//   req.set_latitude(lat_val);
//
//   auto call = std::make_unique<UpdateVehicleLocationCallData>();
//   call->ctx.set_deadline(std::chrono::system_clock::now() +
//                          std::chrono::milliseconds(client_duration));
//
//   call->response_reader =
//       stub->AsyncUpdateVehicleLocation(&call->ctx, req, &cq);
...
// }
//
// bool VechileGatewayClient::SendEta(const std::string &vin_number_val,
//                                    int64_t request_id_val, double time_val,
//                                    double fare_val) {
...
// }
//
// bool VechileGatewayClient::SendStatus(const std::string &vin_number_val,
//                                       int64_t trip_id_val,
...
// }
//
// bool VechileGatewayClient::SendArrive(const std::string &vin_number_val,
//                                       int64_t trip_id_val, double long_val,
//                                       double lat_val) {
//   auto start = std::chrono::steady_clock::now();
//
...
// }
//
// void parsing_received_message(const std::string &topic,
//                               const std::string &payload) {
//   try {
//     if (topic == VehicleGatewayConstants::TripInit) {
//       spdlog::info("[TRIP]received message: {}", topic);
//
//       auto json = nlohmann::json::parse(payload);
//
//       auto vin = json["vinNumber"].get<std::string>();
//       auto trip = json["requestId"].get<int64_t>();
//       auto slong = json["startLong"].get<double>();
//       auto slat = json["startLat"].get<double>();
//       auto elong = json["endLong"].get<double>();
//       auto elat = json["endLat"].get<double>();
//
//       spdlog::info("[TRIP] {} , {} , {} , {} , {} , {}", vin, trip, slong,
//       slat,
//                    elong, elat);
//
...
//     if (IsMovement()) {
//       bool status_success = client.SendStatus(
//           trip_data.vinNumber, trip_data.trip_id, trip_data.vehicle_status);
//       if (status_success) {
...
// }
//
// void SetTripState(TripState state) { trip_data.trip_status = state; }
//
// void TripFlow() {
...
//         case TripState::IDLE: {
//           SetTripState(TripState::UPDATE_VEHICLE_LOCATION);
//           break;
//         }
//
//         case TripState::UPDATE_VEHICLE_LOCATION: {
//           trip_data.longitude = 30.8;
//           trip_data.latitude = 30.1;
//
//           bool update_location_success = client.UpdateLocation(
//               trip_data.vinNumber, trip_data.longitude, trip_data.latitude);
//
//           if (update_location_success) {
//             spdlog::info("[TRIP]update location successfully.");
//             SetTripState(TripState::WAIT_INIT);
//           } else {
//             spdlog::info("[TRIP]update location failed");
//           }
//
//           break;
//         }
...
//
//           if (!WaitForMqttMessage(VehicleGatewayConstants::TripInit)) {
...
//           spdlog::info(
//               "[TRIP]calling autoware for whole trip path planning....");
//
//           // calling autoware
//
...
//
//           bool eta_success =
//               client.SendEta(trip_data.vinNumber, trip_data.trip_id,
//                              trip_data.time, trip_data.fare);
//           if (!eta_success) {
...
//
//           if (!WaitForMqttMessage(VehicleGatewayConstants::TripMove)) {
...
//           bool arrive_sucess =
//               client.SendArrive(trip_data.vinNumber, trip_data.trip_id,
//                                 trip_data.lon, trip_data.lat);
//           if (!arrive_sucess) {
